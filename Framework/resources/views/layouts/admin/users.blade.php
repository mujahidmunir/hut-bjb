...
@section('head')
    @include('layouts.assets.component.data-table.head')
@endsection
@section('js')
    @include('layouts.assets.component.data-table.js')
    <script>
        $.get("{{url('api/users')}}", function (data, status) {
            var res = [];
            data.map(function (v, i) {
                ...
                // }
                res.push([i + 1, v.name, v.email, '<a href="#" data-toggle="modal" data-target="#changepassword" class="btn btn-primary btn-sm" onclick="cp(' + v.nik + ')">Modal</a>'])
            });
            $('#myuser').DataTable({
                lengthChange: true,
                data: res,
