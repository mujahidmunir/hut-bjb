...
@section('js')
    ...
    <script>
        $.get("{{url('api/admin/get-program')}}", function (data) {
            var res = [];
            console.log(data);
            data.map(function (v, i) {
                ...
                // }
                res.push([i + 1,v.title, v.cat_name,v.location,v.seen,v.cabang,v.pic_name,'0'+v.no_wa, '<a href="#" data-toggle="modal" data-target="#changepassword" class="btn btn-primary btn-sm" onclick="cp(' + v.nik + ')">Modal</a>'])
            });
            $('#myuser').DataTable({
                lengthChange: true,
                data: res,
